...
int sumOfLeftLeaves(TreeNode* root) {
    if(!root) return 0;
    int res = 0;
    if(root->left) res += isLeafNode(root->left)? root->left->val: sumOfLeftLeaves(root->left);
    if(root->right && !isLeafNode(root->right)) res += sumOfLeftLeaves(root->right);
    return res;
}
...
TreeNode* deleteTreeNode(TreeNode* root, int key) {
    if(!root) return NULL;
    if(root->val > key) root->left = deleteTreeNode(root->left, key);
    else if(root->val < key) root->right = deleteTreeNode(root->right, key);
    else {
